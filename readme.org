...
Modern dired replacement with sidebar mode. =[f9]= toggles a side panel like neotree did.
#+begin_src elisp
  (use-package dirvish
    :ensure t
    :after nerd-icons
...
#+end_src
** Treesiter
Use the built-in Emacs 29+ grammar installer. Run =M-x treesit-install-language-grammar= per language, or evaluate ~(mapc #'treesit-install-language-grammar (mapcar #'car treesit-language-source-alist))~ to install all at once.
#+begin_src elisp
  (require 'treesit)
...
On first run install dictionaries (e.g. =brew install hunspell= and place pt_BR + en_US dicts in =~/Library/Spelling/=).
#+begin_src elisp
  (use-package jinx
    :ensure t
...
* Magit
Magit requires transient >= 0.8.8, but Emacs ships an older built-in version. We install transient explicitly so Magit picks the newer one.
#+begin_src elisp
...
#+end_src
** Apheleia (formatting)
Async formatting that doesn't fight the cursor. Auto-detects prettier, biome, rubocop, etc. from project config.
#+begin_src elisp
...
#+end_src
** ts-comint
First, we need xterm-color for proper ANSI color handling in comint buffers:
#+begin_src elisp
...
#+end_src
** Mini-frame
Centered child-frame for any minibuffer prompt (M-x, find-file, vertico completion, etc). Replaces the previous vertico-posframe setup.
#+begin_src elisp
...
#+end_src
* Local Config
Per-machine overrides live in =~/.emacs.d/local.el= (gitignored). See =local.el.example= for a template. Loaded last so any setting here overrides the base config.
#+begin_src elisp
